import type {
  AddCocktailToListsSubmission,
  BarDetails,
  CocktailSubmission,
  CreateAccountSubmission,
  LoginSubmission,
  ReviewSubmission,
} from './models';
...
const makeCall = async (endpoint: string, options: any, isFormData?: boolean) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Response status: ${response.status}. ${errorMessages.length > 0 ? errorMessages.join('; ') : ''}`,
    );
  }
